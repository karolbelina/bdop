SELECT pseudo AS "PSEUDO", w_stadku_od AS "W STADKU",
       ...
       AND MOD(TO_CHAR(LAST_DAY(SYSDATE), 'DAY', 'NLS_DATE_LANGUAGE=''numeric date language''') + 4, 7) < LAST_DAY(SYSDATE) - SYSDATE
       /* the function for the last wednesday of the current month is 
          x - (x + 4) mod 7, where x is the last day of the month */
       THEN LAST_DAY(SYSDATE) - MOD(TO_CHAR(LAST_DAY(SYSDATE), 'DAY', 'NLS_DATE_LANGUAGE=''numeric date language''') + 4, 7)
       /* to get the last wednesday of the next month just pass the
          LAST_DAY of the LAST_DAY + 1 to the function mentioned above */
       ELSE LAST_DAY(LAST_DAY(SYSDATE) + 1) - MOD(TO_CHAR(LAST_DAY(LAST_DAY(SYSDATE) + 1), 'DAY', 'NLS_DATE_LANGUAGE=''numeric date language''') + 4, 7)
       END AS "WYPLATA"
  FROM kocury;
